...
2. 导入表格并执行 Preflight。
3. 下载源视频。
4. 以本地视觉、动作、音频连续性优先自动分割；计算区间强制为 `5-60s`，推荐为 `5-30s`。
5. 合法片段进入 `AfterEdit`，异常片段进入 `ProblemClips`。
6. 对合法片段执行视频级打标。
7. 多分支回表；新 V0.3 任务按唯一合法 `核心动作/主动作` 的完整路径归档。V0.1、V0.2 和历史显式内容路径保持兼容。
8. 展示结果并导出失败 CSV。
...
- 回表和 sidecar 保留次动作与其他已接受标签。新 V0.3 任务的归档只消费唯一合法 `核心动作/主动作`；校验失败时最多修复一次，仍失败则待复核且不归档。旧路径和已有文件不自动迁移。
