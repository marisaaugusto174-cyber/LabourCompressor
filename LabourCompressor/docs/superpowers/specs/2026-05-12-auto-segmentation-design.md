...
- 在打标前自动分割已下载视频。
- 自动分割后覆盖约 `80%` 的有效源内容，而不是只挑选高光。
- 允许丢弃片头片尾纯海报、空白转场、极短孤立片段，以及自动分割过程中的少量剪辑损耗。
...
其中“检测结果异常”覆盖源媒体无法读取、候选切点检测失败、分割结果为空等检测链路无法产生可信结果的情况。第一版不引入“内容不明所以”这类模型判断，否则会拖慢自动化链路并扩大不确定性。
...
`片段秒数` 使用分割后片段自身时长，不使用原始完整视频时长。数字后缀表示该片段在源视频中的顺序。
...
- 使用一条 2-3 分钟商业广告；
- 验证所有最终片段都在 `3-30s`；
- 验证约 `80%` 有效源内容被覆盖，片头片尾纯海报和少量剪辑损耗可接受；
...
这个默认值优先保证自动化和约 `80%` 的有效内容覆盖。只有系统无法导出合法媒体、无法让结果满足 `3-30s`，或检测结果异常时，才进入 `ProblemClips`。
