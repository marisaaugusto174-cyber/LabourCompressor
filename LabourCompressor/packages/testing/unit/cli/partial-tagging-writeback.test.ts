...
test('prefers manual accepted sidecar over model candidate json', async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), 'labour-compressor-partial-writeback-accepted-'));
  const afterEditDirectory = path.join(tempDir, 'AfterEdit');
  const mediaPath = path.join(afterEditDirectory, '样本C_720P_260610_000004_01.mp4');
  const spreadsheetPath = path.join(afterEditDirectory, 'AfterEdit_归档记录表.xlsx');
  const taxonomyPath = path.join(tempDir, 'taxonomy.md');
  const promptPath = path.join(tempDir, 'prompt.md');
  ...
  try {
    ...
    writeFileSync(
      mediaPath.replace(/\.mp4$/u, '.accepted.json'),
      `${JSON.stringify({
        version: 1,
        reviewItemId: 'review-manual',
        videoRelativePath: '样本C_720P_260610_000004_01.mp4',
        sourceJsonRelativePath: '样本C_720P_260610_000004_01.json',
        taxonomyVersion: 'Core_Prompt_V0.1',
        taxonomyChecksum: 'sha256:test-taxonomy',
        status: '通过',
        acceptedPaths: ['内容领域 > 娱乐文化 > 娱乐领域 > 游戏电竞'],
        source: 'manual',
    ...
    const worksheet = xlsx.utils.aoa_to_sheet([
      ['文件名', '相对路径', '归档状态', '源文件路径', '当前文件路径'],
      ['样本C_720P_260610_000004_01.mp4', '样本C_720P_260610_000004_01.mp4', '已压缩', mediaPath, mediaPath]
    ]);
    xlsx.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
    xlsx.writeFile(workbook, spreadsheetPath);
    ...
    assert.equal(records[0]?.三级标签, '内容领域: 游戏电竞');
    assert.equal(records[0]?.归档路径, '视频数据归档库/内容领域/娱乐文化/娱乐领域/游戏电竞');
    assert.equal(records[0]?.标签JSON文件, '样本C_720P_260610_000004_01.accepted.json');
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
